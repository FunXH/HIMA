...
class Trainer(TrainBase):
    ...
    def _ihir_arg(self, name: str, default, legacy_name: str = None):
    ...
    def _ihir_soft_binary(self, h: torch.Tensor) -> torch.Tensor:
        return torch.tanh(h)

    def _ihir_match_dim(self, a: torch.Tensor, b: torch.Tensor):
        dim = min(a.shape[-1], b.shape[-1])
        return a[..., :dim], b[..., :dim]

    def _ihir_hamming_distance(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        a, b = self._ihir_match_dim(a, b)
        if b.dim() == 3 and a.dim() == 2:
            a = a.unsqueeze(1)
        return 0.5 * (1.0 - (a * b).mean(dim=-1))

    def _ihir_select_reliable_anchors(
        self,
        ...
        top_k = top_k or self._ihir_arg("ihir_topk", self._ihir_arg("token", 4), legacy_name="hima_topk")
        top_k = max(1, min(int(top_k), instance_hashes.shape[1]))

        soft_instances = self._ihir_soft_binary(instance_hashes.detach())
        soft_target = self._ihir_soft_binary(holistic_target_hash.detach()).unsqueeze(1)
        soft_instances, soft_target = self._ihir_match_dim(soft_instances, soft_target)
    ...
    def _ihir_multiscale_loss(
        self,
        adv_hash: torch.Tensor,
        holistic_target_hash: torch.Tensor,
        reliable_anchors: torch.Tensor,
    ):
        lam = float(self._ihir_arg("ihir_lambda", 1.0, legacy_name="hima_lambda"))
        margin = float(self._ihir_arg("ihir_margin", 0.1, legacy_name="hima_margin"))

        soft_adv = self._ihir_soft_binary(adv_hash)
        soft_target = self._ihir_soft_binary(holistic_target_hash.detach())
        soft_anchors = self._ihir_soft_binary(reliable_anchors.detach())

        loss_holistic = self._ihir_hamming_distance(soft_adv, soft_target).mean()
        anchor_dist = self._ihir_hamming_distance(soft_adv, soft_anchors)
    ...
    def _ihir_instance_set_anchor_loss(
        self,
        ...
        soft_adv_instances = self._ihir_soft_binary(adv_instance_hashes)
        soft_anchors = self._ihir_soft_binary(reliable_anchors.detach())
        soft_adv_instances, soft_anchors = self._ihir_match_dim(soft_adv_instances, soft_anchors)
        pairwise_dist = 0.5 * (
            1.0 - (soft_adv_instances.unsqueeze(2) * soft_anchors.unsqueeze(1)).mean(dim=-1)
        )
        return pairwise_dist.min(dim=2).values.mean()

    def _ihir_text_multiscale_loss(
        self,
        ...
        loss_total, loss_holistic, loss_anchor = self._ihir_multiscale_loss(
        ...
        text_instance_weight = float(
            self._ihir_arg("ihir_text_instance_lambda", 0.5, legacy_name="hima_text_instance_lambda")
        )
        loss_text_instance = self._ihir_instance_set_anchor_loss(
    ...
    def _ihir_target_text_anchors(
        ...
        return self._ihir_select_reliable_anchors(target_text_instances, holistic_target_hash)

    def _ihir_target_image_anchors(
        self,
        ...
        return self._ihir_select_reliable_anchors(target_image_instances, holistic_target_hash)
    ...
    def image_attack(
        ...
        reliable_anchors = self._ihir_target_text_anchors(
            image=orig_image,
        ...
        for i in range(num_iter):
            ...
            loss, loss_holistic, loss_instance = self._ihir_multiscale_loss(
                adv_hash=image_hash,
                holistic_target_hash=target_hash,
                reliable_anchors=reliable_anchors,
            )
    ...
    def text_attack(
        ...
        reliable_anchors = self._ihir_target_image_anchors(
        ...
        for i in range(num_iter):
            ...
            text_instance_hashes = self._get_ihir_text_instance_hashes(adv_embed)

            total_loss, loss_holistic, loss_instance, loss_text_instance = self._ihir_text_multiscale_loss(
    ...
    def _resize_text_tokens_for_ihir_probe(self, token_features: torch.Tensor) -> torch.Tensor:
    ...
    def _get_ihir_text_instance_hashes(self, adv_embed: torch.Tensor) -> torch.Tensor:
        token_features = self._get_text_token_features(adv_embed)
        token_features = self._resize_text_tokens_for_ihir_probe(token_features)
