...
@registry.register_runner("DNPHTrainer")
class DNPHTrainer(BaseTrainer):
    def __init__(self,
                cfg, 
                is_train=True, 
                logger=None, 
    ...
    def from_config(cls, rank=0, world_size=torch.cuda.device_count(), distributed=False, cfg=None, logger=None):
        ...
        output_dim = cfg.run.get("output_dim", 64)
    ...
    def build_optimizer(self, cfg_optimizer=None, parameters=None):
        ...
        lr_ = cfg_optimizer.loss.get("lr", 0.0001)
        optimizer_loss = torch.optim.SGD(params=self.model.loss.parameters(), lr=lr_)
    ...
    def compute_loss(self, img_hash=None, txt_hash=None, img_pre=None, txt_pre=None, label=None, index=None, epoch=0, times=0, global_step=0, **kwags):
        
        all_loss, loss_dict = self.model.object_function(img_hash=img_hash, txt_hash=txt_hash, img_pre=img_pre, txt_pre=txt_pre, labels=label, indexs=index, **kwags)
    ...
    def train_epoch(self, epoch: int):
        ...
        for image, text, key_padding_mask, label, index in self.train_loader:
            ...
            img_hash, txt_hash, img_pre, txt_pre = self.model_ddp(image, text, return_loss=False) if self.model_ddp is not None else self.model(image, text, return_loss=False) 
            loss = self.compute_loss(img_hash=img_hash, txt_hash=txt_hash, img_pre=img_pre, txt_pre=txt_pre, label=label, index=index, epoch=epoch, times=times, global_step=self.global_step)
