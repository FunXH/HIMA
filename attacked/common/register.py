...
class Registry:
    ...
    def register_dataset(cls, name):
        ...
        def wrap(dataset_cls):
            ...
            ), "All builders must inherit BaseDataset class, found {}".format(
                dataset_cls
            )
    ...
    def register_optimizer(cls, name):
        ...
        def wrap(optimizer_cls):
            ...
            if name in cls.mapping["optimizer_name_mapping"]:
                raise KeyError(
                    "Name '{}' already registered for {}.".format(
                        name, cls.mapping["optimizer_name_mapping"][name]
                    )
                )
            cls.mapping["optimizer_name_mapping"][name] = optimizer_cls
    ...
    def register_tokenizer(cls, name):
        ...
        def wrap(tokenizer_cls):
            if name in cls.mapping["tokenizer"]:
                raise KeyError(
                    "Name '{}' already registered for {}.".format(
                        name, cls.mapping["tokenizer"][name]
                    )
                )
            cls.mapping["tokenizer"][name] = tokenizer_cls
    ...
    def register_runner(cls, name):
        ...
        def wrap(runner_cls):
            from runners.base import BaseTrainer

            assert issubclass(
                runner_cls, BaseTrainer
            ), "All trainer must inherit BaseTrainer class"
    ...
    def get_tokenizer_class(cls, name):
        return cls.mapping["tokenizer"].get(name, None)
    ...
    def list_optimizer(cls):
        return sorted(cls.mapping["optimizer_name_mapping"].keys())
