...
class AdaptiveLRController:
    ...
    功能：
    1. 当连续patience次步长的指标变化小于threshold，自动衰减学习率
    2. 提供最小值约束防止学习率过小

    参数：
    - init_alpha: 初始学习率 (float)
    - patience: 观察窗口大小 (int)
    - threshold: 视为停滞的阈值 (float)
    - gamma: 衰减系数 (0~1)
    - min_alpha: 最小学习率 (float)
    """
    ...
    def __init__(self,
        ...
        self.history_loss = deque(maxlen=patience + 1)

    def step(self, loss_val: float) -> float:
        """更新学习率状态并返回当前学习率
        ...
        # 计算最后一次变化量
        delta = abs(self.history_loss[-1] - self.history_loss[-2])

        # 当连续patience次满足阈值，执行衰减
        if delta < self.threshold and len(self.history_loss) > self.patience:
            # 检查最后patience次的变化是否均低于阈值
            all_stable = all(
                abs(self.history_loss[i] - self.history_loss[i - 1]) < self.threshold
                for i in range(-self.patience + 1, 0)
            )

            if all_stable:
                # 衰减学习率
                new_alpha = max(self.current_alpha * self.gamma, self.min_alpha)
                # 重置记录
