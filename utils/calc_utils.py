...
#     # print("hamms")
#     #大规模的hamms矩阵排序会造成大量的内存占用，用下面的函数将gnds和hamms矩阵分batch处理，降低内存的需求防止oom
...
def calc_map_k_matrix(qB, rB, query_L, retrieval_L, k=None, rank=0):
    num_query = query_L.shape[0]
    # 确保数据在CPU
    qB = qB.cpu() if qB.is_cuda else qB
    rB = rB.cpu() if rB.is_cuda else rB
    query_L = query_L.cpu()
    ...
    map = 0
    # --------- Batch processing query_L ---------
    batch_size = 1000  # 根据内存调整此值
    for start in range(0, num_query, batch_size):
        end = min(start + batch_size, num_query)
        batch_query_L = query_L[start:end]
        # Calculate in batches gnds
        gnds_batch = (batch_query_L.mm(retrieval_L.transpose(0, 1)) > 0).type(torch.float32)
        tsums_batch = torch.sum(gnds_batch, dim=-1, dtype=torch.int32)
        totals_batch = torch.min(tsums_batch, torch.tensor(k, dtype=torch.int32))

        qB_batch = qB[start:end]
        hamms_batch = calc_hammingDist(qB_batch, rB)
        _, ind_batch = torch.sort(hamms_batch, dim=-1)

        # Sample-by-sample processing
        for i in range(gnds_batch.shape[0]):
            gnd = gnds_batch[i][ind_batch[i]]
            total = totals_batch[i].squeeze()
            if total == 0:
                continue  # Avoiding Division by Zero Errors
            count = torch.arange(1, total + 1).float()
            tindex = torch.nonzero(gnd)[:total].squeeze().float() + 1.0
            if tindex.dim() == 0:  # Handling single element cases
                tindex = tindex.unsqueeze(0)
            map += torch.mean(count / tindex).item()
...
def calc_soft_map_k(qB, rB, query_L, retrieval_L, k=None, rank=0):
    """
    Weighted T-mAP for multi-label retrieval

    Args:
        qB: query hash codes [Nq, bit]
        rB: retrieval hash codes [Nr, bit]
        query_L: query/target labels [Nq, L]
        retrieval_L: retrieval labels [Nr, L]
        k: top-k
        rank: device id
    ...
    for i in range(num_query):
        q_L = query_L[i]
        if len(q_L.shape) < 2:
            q_L = q_L.unsqueeze(0)  # [1, L]

        # ===== Step 1: 计算标签重合数量 =====
        overlap = q_L.mm(retrieval_L.transpose(0, 1)).squeeze()  # [Nr]

        # ===== Step 2: binary relevance（保证MAP稳定）=====
        binary_gnd = (overlap > 0).float()

        if binary_gnd.sum() == 0:
            continue

        # ===== Step 3: weighted relevance（关键！）=====
        # 用匹配比例作为权重（比Jaccard稳定）
        norm = q_L.sum() + 1e-6
        gnd = binary_gnd * (overlap / norm)
        ...
        # ===== Step 6: Soft-AP计算 =====
        cumsum = torch.cumsum(gnd, dim=0)
        denom = torch.arange(1, len(gnd) + 1).float()

        if gnd.is_cuda:
            denom = denom.to(gnd.device)

        precision = cumsum / denom

        AP = (precision * gnd).sum() / (gnd.sum() + 1e-6)
