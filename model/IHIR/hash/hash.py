...
from attacked.models.common.hash import Hash, softmax_hash, tanh_hash, weights_init_kaiming
...
class TokenHash(Hash):
    ...
    def forward(self, embeds, global_embed=None):
        # 确保输入与卷积权重类型相同
        if embeds.dtype != self.token_layer.weight.dtype:
            embeds = embeds.to(self.token_layer.weight.dtype)
        ###
        # B,T,L -> B,M,L
