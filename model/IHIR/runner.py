...
@registry.register_runner("IHIRTrainer")
class IHIRTrainer(BaseTrainer):
    def __init__(self,
                cfg, 
                is_train=True, 
                logger=None, 
    ...
    def build_model(self, cfg_model, output_dim=16, txt_token_size=32):
        ...
        # print(arch)
        self.model = registry.get_model_class(arch).from_config(
            cfg_model,
            output_dim=output_dim,
            training_sample_count=self.train_num,
            text_token_count=txt_token_size,
        )
